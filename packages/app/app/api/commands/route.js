...
export async function POST(request) {
  try {
    ...
    const { data: command, error: commandError } = await supabase.from('timeoe_commands').insert({ objective, business_id: businessId, status: 'QUEUED', plan: { source: 'timeoe-command-center' } }).select().single();
    ...
    // Start the deterministic execution kernel immediately. External side effects remain behind integrations/approval gates.
    const origin = request.headers.get('origin') || request.nextUrl.origin;
    fetch(`${origin}/api/commands/${command.id}/execute`, { method: 'POST' }).catch(() => {});
