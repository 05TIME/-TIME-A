import { supabaseAdmin, emitBusinessEvent } from './business-actions';
...
export async function recordVerifiedRevenue({ action, amount, currency = 'USD', occurredAt = new Date().toISOString() }) {
  ...
  const { data: transaction, error: transactionError } = await supabaseAdmin.from('transactions').insert({ business_id: action.business_id, type: 'INCOME', amount: value, category: 'TIMEOE_REVENUE', description, occurred_at: occurredAt }).select().single();
  ...
  const snapshot = { business_id: action.business_id, day, cash: Number(previous?.cash || 0) + value, revenue, expenses: Number(previous?.expenses || 0), customers: Number(previous?.customers || 0), leads: Number(previous?.leads || 0), operating_score: Number(previous?.operating_score || 0), created_at: new Date().toISOString() };
  const snapshotResult = previous ? await supabaseAdmin.from('kpi_snapshots').update(snapshot).eq('id', previous.id).select().single() : await supabaseAdmin.from('kpi_snapshots').insert(snapshot).select().single();
  if (snapshotResult.error) throw snapshotResult.error;
  const { error: activityError } = await supabaseAdmin.from('activity_log').insert({ business_id: action.business_id, actor_id: null, event_type: 'TIMEOE_REVENUE_VERIFIED', message: `Verified revenue recorded from business action ${action.id}`, metadata: { action_id: action.id, transaction_id: transaction.id, amount: value, currency, provider: action.provider } });
