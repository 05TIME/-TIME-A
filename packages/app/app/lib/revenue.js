...
export async function recordVerifiedRevenue({ action, amount, currency = 'USD', occurredAt = new Date().toISOString() }) {
  ...
  if (action.payload?.simulation === true || action.payload?.test_mode === true) throw new Error('Simulation/test actions cannot create live revenue');

  const description = `TIMEOE revenue | action:${action.id} | provider:${action.provider}`;
  const { data: existing, error: existingError } = await supabaseAdmin.from('transactions').select('*')
  ...
  const snapshot = {
    business_id: action.business_id, day, cash: Number(previous?.cash || 0) + value, revenue,
    expenses: Number(previous?.expenses || 0), customers: Number(previous?.customers || 0),
    leads: Number(previous?.leads || 0), operating_score: Number(previous?.operating_score || 0),
  ...
  await supabaseAdmin.from('timeoe_business_actions').update({
    verified_at: new Date().toISOString(), verification: { verified_sale: true, transaction_id: transaction.id, amount: value, currency }
  }).eq('id', action.id);
  await emitBusinessEvent(action.command_id, action.task_id, 'REVENUE_VERIFIED', 'VERIFIED', {
    action_id: action.id, transaction_id: transaction.id, amount: value, currency, revenue_total: revenue
  });
